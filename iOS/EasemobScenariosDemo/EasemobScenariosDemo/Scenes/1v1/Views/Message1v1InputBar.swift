...
final class Message1v1InputBar: UIView {
    ...
    public private(set) lazy var send: UIButton = {
        UIButton(type: .custom).frame(CGRect(x: self.frame.width - 42, y: self.inputField.frame.maxY-32, width: 30, height: 30)).backgroundColor(.clear).image(UIImage(named: "airplane", in: .chatBundle, with: nil), .normal).addTargetFor(self, action: #selector(sendMessage), for: .touchUpInside)
    }()
    ...
    required public init(frame: CGRect,text: String? = nil,placeHolder: String? = nil) {
        ...
        self.inputField.placeHolder = Appearance.chat.inputPlaceHolder
...
extension Message1v1InputBar: UITextViewDelegate {
    ...
    @objc func updateHeight() {
        ...
        if textHeight >= self.rawTextHeight {
            ...
            if textHeight > Appearance.chat.maxInputHeight {
                self.frame = CGRect(x: 0, y: ScreenHeight - (Appearance.chat.maxInputHeight+16) - self.keyboardHeight, width: self.frame.width, height: Appearance.chat.maxInputHeight+16)
                self.inputField.frame = CGRect(x: 12, y: 8, width: self.frame.width-111, height: Appearance.chat.maxInputHeight)
            } else {
    ...
    @objc func keyboardWillHide(notification: Notification) {
        ...
        if self.emoji == nil {
            let emoji = MessageInputEmojiView(frame: CGRect(x: 0, y: self.inputField.frame.maxY+8, width: self.frame.width, height: self.keyboardHeight)).tag(124)
            self.emoji = emoji
     ...
     */
    @objc func convertText(text: NSAttributedString?, key: String) -> NSAttributedString {
...
extension Message1v1InputBar: ThemeSwitchProtocol {
    ...
    public func switchTheme(style: ThemeStyle) {
        ...
        var image = UIImage(named: "airplane", in: .chatBundle, with: nil)
        if style == .dark {
